  ...
  registerOrgan({
    ...
    handler: async (_organ, session) => {
      ...
      if (goal.includes('create') && (goal.includes('file') || goal.includes('write'))) {
        // Extract filename and content from the original owner statement
        const originalStatement = session.compiledIntent?.originalStatement ?? '';
        const filenameMatch = originalStatement.match(/(?:create|write|make)\s+(?:a\s+)?(?:file\s+)?(?:named\s+)?["']?([^\s"'\.]+)["']?/i);
        const filename = filenameMatch?.[1] ?? 'output.txt';
        const targetFile = join(workspace, filename);
        // Extract content from quoted strings in the original statement
        const contentMatch = originalStatement.match(/containing\s+["'](.+?)["']|with\s+(?:the\s+)?(?:message|content)\s+["'](.+?)["']|["'](.+?)["']/i);
        const extractedContent = contentMatch?.[1] ?? contentMatch?.[2] ?? contentMatch?.[3];
        const content = extractedContent ?? `Generated by GSPL agent for objective: ${intent.goal}`;
  ...
  async function restoreSession(agentId: string): Promise<AgentSession> {
    ...
    if (Array.isArray(state.claims)) {
      for (const claim of state.claims) {
        ...
        } catch (claimErr) { observability.log({ level: 'WARN', source: 'runtime-coordinator', message: 'Restore claim creation failed', sessionId: agentId, tickNumber: 0, correlationId: '', data: { error: claimErr instanceof Error ? claimErr.message : 'unknown' } }); }
      }
    }
    ...
    if (Array.isArray(state.capabilities)) {
      for (const cap of state.capabilities) {
        if (cap && typeof cap === 'object') {
          const c = cap as any; capabilityManager.grant({ name: "restore-" + String(c.effectType ?? 'FILESYSTEM_READ'), effectType: (c.effectType ?? 'FILESYSTEM_READ') as any, scope: c.scope ?? {}, authority: "OWNER", requestedBy: "restore" });
        }
      }
    }
    ...
    if (Array.isArray(state.events)) {
      for (const evt of state.events) {
        ...
        } catch (evtErr) { observability.log({ level: 'WARN', source: 'runtime-coordinator', message: 'Restore event append failed', sessionId: agentId, tickNumber: 0, correlationId: '', data: { error: evtErr instanceof Error ? evtErr.message : 'unknown' } }); }
      }
    }
