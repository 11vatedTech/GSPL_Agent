  ...
  registerOrgan({
    ...
    handler: async (_organ, session) => {
      ...
      if (fileOp) {
        ...
        const content = fileOp.requestedContent ?? `Generated by GSPL agent for objective: ${intent.goal}`;
        if (!filename) {
          return {
            output: { planGenerated: false, reason: 'No resolvable artifact path in compiled intent' },
            confidence: 0, evidence: [],
            errors: [{ code: 'UNRESOLVED_INTENT', message: 'Artifact operation missing required path', severity: 'error', recoverable: true }],
            consumedResources: { computeUnits: 0, memoryBytes: 0 },
          };
        }
